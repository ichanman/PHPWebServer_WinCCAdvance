...
if (isset($_GET['runBatch']) && $_GET['runBatch'] === 'true' && !isset($_GET['print'])) {
	...
	$taskCommand = 'schtasks /create /sc once /tn "' . $taskName . '" /tr "' . $batFilePath . ' \'' . $escapedUrl2 . '\' " /st ' . date("H:i", strtotime('+1 minute')) . ' /f /RU "vboxuser"';
	...
	<div class="container">
		<div class="filter-container">
			<div id="filter-buttons">
				...
				<!-- Sort buttons placed next to Apply Range -->
				<a
					href="index.php?filter=<?= $filter ?>&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>&order=ASC">Sort
					Ascending</a> |
				<a
					href="index.php?filter=<?= $filter ?>&start_date=<?= $startDate ?>&end_date=<?= $endDate ?>&order=DESC">Sort
					Descending</a>
			</div>
		</div>
